...
import { useCityData, CityComponent } from './smartcity/useCityData'
...
export default function SmartCity3D({ activeAttack, onAttackEnd }: SmartCity3DProps) {
  const { components, loading, error } = useCityData(activeAttack ? 2000 : 5000)
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [hoveredComponent, setHoveredComponent] = useState<CityComponent | null>(null)
  ...
  const handleHover = useCallback((comp: CityComponent | null, event?: ThreeEvent<PointerEvent>) => {
    ...
    return (
      <div style={containerStyle}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
          ...
          <span style={{ color: 'var(--text-secondary)', marginLeft: '1rem' }}>Loading 3D City...</span>
        </div>
      </div>
    )
  }
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        <div style={legendStyle}>
          ...
          {[
            ...
            const count = components.filter((c) => c.zone === zone).length
        ...
        {selectedComponent && !activeAttack && !showWindmillPanel && (
          <div ref={detailsRef} style={detailsPanelStyle}>
            ...
            <div style={{ display: 'grid', gap: '0.5rem' }}>
              <DetailRow label="ID" value={selectedComponent.id} />
              <DetailRow label="Category" value={selectedComponent.category} />
              <DetailRow label="Zone" value={selectedComponent.zone} />
              <DetailRow
                label="Status"
                value={
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                    <span
                      style={{
                        ...
                        height: 8,
                        borderRadius: '50%',
                        background: STATUS_COLORS[selectedComponent.status],
                        display: 'inline-block',
                      }}
                    />
              ...
              />
              <DetailRow label="Events" value={selectedComponent.eventsCount.toLocaleString()} />
              <DetailRow label="Alerts" value={
                <span style={{ color: selectedComponent.alertsCount > 0 ? 'var(--accent-danger)' : 'var(--text-primary)' }}>
                  {selectedComponent.alertsCount}
                </span>
              } />
              <DetailRow
                label="Last Updated"
                value={new Date(selectedComponent.lastUpdated).toLocaleTimeString()}
              />
            </div>
          </div>
        )}
...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <span style={{ color: 'var(--text-tertiary)', fontSize: '0.75rem' }}>{label}</span>
