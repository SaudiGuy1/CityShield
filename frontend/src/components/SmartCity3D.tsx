/**
 * SmartCity3D: 3D Visualization Component (OVERVIEW Mode)
 *
 * This component provides OVERVIEW mode visualization using the CityScene
 * infrastructure with professional HighTopo-grade rendering.
 *
 * Key features:
 * - Shows status-encoded buildings representing asset groups (not individual assets)
 * - Buildings use height to encode risk score
 * - Buildings use color to encode status
 * - Uses CityScene → Zones → StatusBuilding rendering pipeline
 * - Professional lighting: 6-light setup + bloom + SSAO
 * - Optimized for high-level monitoring and attack visualization
 *
 * Attack visualization:
 * - Real-time attack phases with progress tracking
 * - Attack path visualization overlay
 * - Zone-based targeting indicators
 * - Stage polling for detailed attack progress
 *
 * This is used by the Overview/Dashboard page for executive monitoring.
 */
...
export default function SmartCity3D({ activeAttack, onAttackEnd, isAdmin }: SmartCity3DProps) {
  ...
  const selectedComponent = useMemo(
    () => assets.find((c: CityAsset) => (c.id || c.asset_id) === selectedId) || null,
    [assets, selectedId]
  )
  ...
  const stats = useMemo(() => {
    ...
    const critical = assets.filter((c: CityAsset) => c.status === 'critical').length
    const warning = assets.filter((c: CityAsset) => c.status === 'warning').length
    const ok = assets.filter((c: CityAsset) => c.status === 'ok').length
    const offline = assets.filter((c: CityAsset) => c.status === 'offline').length
  ...
  return (
    <div style={activeContainerStyle}>
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.7rem' }}>
              <span
                style={{
                  ...
                  height: 8,
                  borderRadius: '50%',
                  background: STATUS_COLORS[(hoveredComponent.status || 'ok') as keyof typeof STATUS_COLORS],
                }}
              />
        ...
        {selectedComponent && !activeAttack && (
          <AssetInspectorPanel
