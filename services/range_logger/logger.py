"""
Cyber Range network logger.

Captures traffic on the cyber_range_net interface using tcpdump,
parses packets, and writes CityShield-compatible JSON events to
/data/logs/cyber_range.log for Filebeat to pick up.
"""
import subprocess
import json
import re
...
LOG_PATH = "/data/logs/cyber_range.log"
CAPTURE_INTERFACE = os.getenv("CAPTURE_INTERFACE", "eth0")

# Regex to parse tcpdump verbose output
# Example: "12:34:56.789 IP 172.18.0.3.45678 > 172.18.0.2.80: Flags [S], ..."
TCP_RE = re.compile(
    r"(?P<time>\S+)\s+IP\s+(?P<src>[0-9.]+)\.(?P<sport>\d+)\s+>\s+(?P<dst>[0-9.]+)\.(?P<dport>\d+):\s+(?P<rest>.*)"
)
# ICMP: "12:34:56.789 IP 172.18.0.3 > 172.18.0.2: ICMP echo request, ..."
ICMP_RE = re.compile(
...
def classify_event(src_port: int, dst_port: int, flags: str) -> tuple:
    ...
    # SYN-only packets to many ports = port scan
...
def main():
    print(f"[range_logger] Starting tcpdump on {CAPTURE_INTERFACE}")
    print(f"[range_logger] Writing events to {LOG_PATH}")
    ...
    def shutdown(signum, frame):
        print("[range_logger] Shutting down...")
        proc.terminate()
        sys.exit(0)
    ...
    for line in proc.stdout:
        ...
        if tcp_match:
            ...
            if packet_count % 100 == 0:
                print(f"[range_logger] Processed {packet_count} packets")
            continue
