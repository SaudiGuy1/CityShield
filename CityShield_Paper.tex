...
\begin{figure}[t]
\centering
\IfFileExists{cityshield_dashboard.png}{%
    \includegraphics[width=\columnwidth]{cityshield_dashboard.png}%
}{%
...
Each role follows a distinct operational workflow designed to mirror real-world security operations center (SOC) responsibilities. Fig.~\ref{fig:workflows} illustrates the three primary role workflows.
...
\begin{tikzpicture}[
    ...
    decbox/.style={diamond, draw, aspect=2, inner sep=1pt, font=\scriptsize, fill=yellow!8},
    arrow/.style={-{Stealth[length=1.8mm]}, thick, gray!70},
    title/.style={font=\small\bfseries, fill=white}
]

% ── RESEARCHER COLUMN ──
\node[title] (rt) at (0, 0) {Researcher};
\node[startbox, below=0.3cm of rt] (r1) {Login};
\node[resbox, below=of r1] (r2) {Dashboard\\Review stats};
\node[resbox, below=of r2] (r3) {Choose Path};
\node[resbox, below left=0.5cm and 0.3cm of r3] (r4a) {Built-in\\Scenario};
\node[resbox, below=of r3] (r4b) {Custom\\Scenario};
\node[resbox, below right=0.5cm and 0.3cm of r3] (r4c) {Research\\Lab};
\node[resbox, below=0.8cm of r4a] (r5a) {Select Target\\Run Scenario};
\node[resbox, below=0.8cm of r4b] (r5b) {ATT\&CK Techniques\\Build Chain};
\node[resbox, below=0.8cm of r4c] (r5c) {Launch Ubuntu\\Container};
\node[resbox, below=1.5cm of r3, yshift=-2.8cm] (r6) {Review Alerts\\by Correlation ID};
\node[resbox, below=of r6] (r7) {Tune Rules\\Thresholds};
\node[resbox, below=of r7] (r8) {Configure\\Auto-Response};
...
% ── ANALYST COLUMN ──
\node[title] (at) at (6.5, 0) {Analyst};
\node[startbox, below=0.3cm of at] (a1) {Login};
\node[anabox, below=of a1] (a2) {Dashboard\\Check alerts};
\node[anabox, below=of a2] (a3) {3D Smart City\\Scan for red};
\node[anabox, below=of a3] (a4) {Alert List\\Filter \& Sort};
\node[anabox, below=of a4] (a5) {Expand Alert\\Details};
\node[anabox, below left=0.5cm and 0.1cm of a5] (a6a) {Analysis Tab\\MITRE info};
\node[anabox, below=of a5] (a6b) {Actions Tab\\block / isolate};
\node[anabox, below right=0.5cm and 0.1cm of a5] (a6c) {Events Tab\\Raw timeline};
\node[decbox, below=1.5cm of a5, yshift=-1.0cm] (a7) {True\\Positive?};
\node[anabox, below left=0.5cm and 0.4cm of a7] (a8a) {Execute\\Response Action};
\node[anabox, below right=0.5cm and 0.4cm of a7] (a8b) {Resolve as\\False Positive};
\node[anabox, below=1.5cm of a7, yshift=-0.5cm] (a9) {Review Audit\\Log \& Verify};
\node[anabox, below=of a9] (a10) {Next Alert\\Continue triage};
...
\draw[arrow] (a6b) -- (a7);
\draw[arrow] (a6c) |- (a7);
\draw[arrow] (a7) -| node[above, font=\tiny, pos=0.3] {Yes} (a8a);
\draw[arrow] (a7) -| node[above, font=\tiny, pos=0.3] {No} (a8b);
\draw[arrow] (a8a) |- (a9);
\draw[arrow] (a8b) |- (a9);
\draw[arrow] (a9) -- (a10);

% ── ADMIN COLUMN ──
\node[title] (dt) at (13, 0) {Administrator};
\node[startbox, below=0.3cm of dt] (d1) {Login};
\node[admbox, below=of d1] (d2) {Dashboard\\Full health check};
\node[admbox, below=of d2] (d3) {Admin Tasks};
\node[admbox, below left=0.5cm and 0.6cm of d3] (d4a) {User Mgmt\\Create/Disable};
\node[admbox, below left=0.5cm and -0.3cm of d3] (d4b) {Device Mgmt\\Status/Logs};
\node[admbox, below right=0.5cm and -0.3cm of d3] (d4c) {Proposal\\Review};
\node[admbox, below right=0.5cm and 0.6cm of d3] (d4d) {Rule\\Oversight};
\node[admbox, below=2.0cm of d3] (d5) {Monitoring};
\node[admbox, below left=0.5cm and 0.3cm of d5] (d6a) {OpenSearch\\Dashboards};
\node[admbox, below=of d5] (d6b) {Metrics\\MTTD / MTTR};
\node[admbox, below right=0.5cm and 0.3cm of d5] (d6c) {Audit Log\\All actions};
\node[admbox, below=2.5cm of d5, yshift=0.2cm] (d7) {Security\\Awareness};
...
\begin{tikzpicture}[
    ...
    every node/.style={font=\small},
    source/.style={rectangle, draw, rounded corners, minimum width=1.8cm, minimum height=0.7cm, fill=blue!8, font=\scriptsize, align=center},
    process/.style={rectangle, draw, rounded corners, minimum width=2.0cm, minimum height=0.7cm, fill=orange!10, font=\scriptsize, align=center},
    decision/.style={diamond, draw, aspect=2.2, inner sep=1pt, font=\scriptsize, fill=yellow!8, align=center},
    store/.style={cylinder, draw, shape border rotate=90, minimum width=1.4cm, minimum height=0.6cm, fill=green!8, aspect=0.25, font=\scriptsize, align=center},
    action/.style={rectangle, draw, rounded corners, minimum width=1.8cm, minimum height=0.7cm, fill=red!10, font=\scriptsize, align=center},
    audit/.style={rectangle, draw, rounded corners, minimum width=1.8cm, minimum height=0.7cm, fill=purple!10, font=\scriptsize, align=center},
    arrow/.style={-{Stealth[length=2mm]}, thick, gray!70},
    darrow/.style={-{Stealth[length=2mm]}, thick, red!60},
    label/.style={font=\tiny, fill=white, inner sep=1pt}
]

% ── Stage 1: Event Sources ──
\node[source] (tsim) {Traffic Sim\\:8001};
\node[source, right=0.4cm of tsim] (isim) {IoT Sim\\:8002};
\node[source, right=0.4cm of isim] (nsim) {Network Emu.\\:8003};
\node[source, right=0.4cm of nsim] (atk) {Attack Engine\\injection};

% ── OpenSearch ──
\node[store, below=0.8cm of $(isim)!0.5!(nsim)$] (os) {OpenSearch\\logs-*};
...
\node[process, below=0.8cm of os] (poll) {Detection Engine\\polls every 30s};
\node[process, right=2.0cm of poll] (eval) {Rule Evaluator};

\draw[arrow] (os) -- node[label, right] {query} (poll);
\draw[arrow] (poll) -- node[label, above] {75 rules} (eval);

% ── Evaluator split ──
\node[process, above right=0.3cm and 1.5cm of eval] (spec) {11 Specialized\\Evaluators};
\node[process, below right=0.3cm and 1.5cm of eval] (gen) {Generic Threshold\\Evaluator (54)};

\draw[arrow] (eval) -| node[label, above, pos=0.3] {21 rules} (spec);
\draw[arrow] (eval) -| node[label, below, pos=0.3] {54 rules} (gen);

% ── Threshold check ──
\node[decision, right=1.5cm of $(spec)!0.5!(gen)$] (thresh) {Threshold\\exceeded?};

\draw[arrow] (spec) -| (thresh);
\draw[arrow] (gen) -| (thresh);

% ── Alert generation ──
\node[store, right=1.5cm of thresh] (alerts) {alerts\\index};
\node[below=0.15cm of thresh, font=\tiny] {No $\rightarrow$ skip};

\draw[arrow] (thresh) -- node[label, above] {Yes} (alerts);

% ── Stage 3: Response Manager ──
\node[process, below=1.5cm of poll] (rmgr) {Response Mgr\\polls every 30s};

\draw[arrow] (alerts) |- (rmgr);

% ── Whitelist check ──
\node[decision, right=1.8cm of rmgr] (wl) {Whitelisted\\IP?};
\draw[arrow] (rmgr) -- (wl);

% ── Policy check ──
\node[decision, right=1.8cm of wl] (policy) {Auto-resp.\\conditions\\met?};
\draw[arrow] (wl) -- node[label, above] {No} (policy);

\node[process, below=0.6cm of wl] (resolve) {Auto-resolve\\alert};
\draw[darrow] (wl) -- node[label, right] {Yes} (resolve);

% ── Playbook execution ──
\node[action, right=1.8cm of policy] (play) {Ansible\\Playbook};
\draw[arrow] (policy) -- node[label, above] {Yes} (play);

\node[below=0.15cm of policy, font=\tiny] {No $\rightarrow$ skip};

% ── Audit ──
\node[audit, below=0.8cm of play] (auditlog) {action-audit-log\\index};
\draw[arrow] (play) -- (auditlog);

% ── Update alert ──
\node[process, left=1.5cm of auditlog] (update) {Update alert\\status: resolved};
\draw[arrow] (auditlog) -- (update);

% ── Frontend ──
\node[source, below=0.8cm of $(rmgr)!0.5!(update)$, fill=teal!10, minimum width=3.0cm] (fe) {React Frontend\\REST + WebSocket};
\draw[arrow] (alerts) |- (fe);
\draw[arrow] (auditlog) |- (fe);

% ── Stage labels ──
\node[font=\scriptsize\bfseries, gray, above=0.15cm of tsim, xshift=-0.3cm] {Stage 1: Generation};
\node[font=\scriptsize\bfseries, gray, left=0.1cm of poll] {Stage 2: Detection};
\node[font=\scriptsize\bfseries, gray, left=0.1cm of rmgr] {Stage 3: Response};
\node[font=\scriptsize\bfseries, gray, left=0.1cm of fe, xshift=-0.2cm] {Stage 4: Presentation};

\end{tikzpicture}
}
\caption{Detection and response pipeline flow. Events from four sources are indexed in OpenSearch. The detection engine evaluates 75 rules using specialized or generic evaluators, generating alerts when thresholds are exceeded. The response manager validates whitelist, auto-response conditions, and executes Ansible playbooks with full audit traceability.}
