...
% ═══════════════════════════════════════════════════════════════════
\begin{abstract}
Smart city infrastructure combines traffic management, IoT sensor networks, and interconnected utility systems into a broad attack surface. Cyber ranges designed for enterprise IT do not adequately represent this surface. We introduce CityShield, a containerized cyber range that brings together smart city simulation, detection aligned with the MITRE ATT\&CK framework, and automated response under per-rule policies in a single deployable platform. The detection engine evaluates 75 YAML rules spanning 13 ATT\&CK tactics through 11 specialized aggregation evaluators and a generic threshold evaluator, all querying centralized OpenSearch log indices. A companion response manager enforces auto-response policies and dispatches 12 containment actions through Ansible orchestration, with full audit traceability. In a controlled adversarial emulation comprising 15 attack campaigns (1,064 attack events plus 51 benign but ambiguous events) against a background of 689,172 simulator-generated events, CityShield triggers all 75 rules, confirming pipeline correctness under ideal injection conditions. The evaluation covers 76 MITRE ATT\&CK techniques across 14 tactics with a 4.2\% false positive rate (90 of 2,134 alerts traced to benign sources). These metrics represent upper-bound performance; real-world detection rates would be lower because of traffic variability, adversarial evasion, and encrypted payloads. The response pipeline executes 461 actions with a mean time to detect (MTTD) of 16.8 seconds and a mean time to respond (MTTR) of 38.4 seconds. When compared against a baseline using only generic thresholds, CityShield's combination of specialized and generic evaluators reduces false positives by 62\% for reconnaissance and exfiltration detection. This is the primary technical contribution of this work. The platform deploys as 17 Docker Compose services across three isolated network segments and is open-source.\footnote{\url{https://github.com/Rin1t/CityShield}}
\end{abstract}
...
Smart cities integrate cyber-physical systems across transportation, energy, water, and public safety domains. A compromised traffic controller can cascade into city-wide gridlock. Breached IoT sensors can feed falsified environmental data to emergency systems. Poorly segmented networks allow lateral movement to critical infrastructure within seconds~\cite{braun2018}. These risks are structural: they arise from the multi-domain interconnectivity that defines smart city infrastructure rather than from any single misconfigured component.

Despite the severity of these risks, the cybersecurity community lacks training and evaluation platforms that capture the heterogeneous nature of smart city environments. General-purpose cyber ranges model enterprise IT or military networks~\cite{davis2013survey, vykopal2017kypo, yamin2020cyber} and do not simulate the interplay between traffic, IoT, and network infrastructure. Physical ICS testbeds such as SWaT~\cite{swat2016} offer high fidelity for specific industrial processes but are neither reproducible without specialized hardware nor representative of the broader smart city ecosystem. Commercial SIEM and SOAR platforms (e.g., Splunk, IBM QRadar) provide detection and orchestration capabilities but do not include traffic generation, attack simulation, or workflows designed for training. The result is that smart city defenders cannot rehearse attack scenarios spanning multiple domains, evaluate detection coverage against established threat frameworks, or validate response playbooks in a realistic but safe environment.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Attack simulation}: Three event generators, one per domain (traffic management, IoT sensors, network infrastructure), together with an attack injection engine covering 31 MITRE ATT\&CK techniques. All feed a centralized OpenSearch data store.
    \item \textbf{Threat detection}: A detection engine that polls three log indices every 30 seconds and evaluates 75 YAML rules using 11 specialized aggregation evaluators alongside a configurable generic threshold evaluator.
    \item \textbf{Automated response}: A response manager that validates conditions on a per-rule basis (severity, enrichment, rate limits, CIDR whitelisting) and dispatches 12 containment actions through Ansible playbooks, logging every action to an append-only audit index.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{A containerized cyber range that models smart city attack surfaces spanning multiple domains, with isolation enforced at the network level.} To our knowledge, no existing containerized cyber range (cf.\ Table~\ref{tab:comparison}) models smart city infrastructure across multiple domains. Unlike ranges built for enterprise IT (KYPO~\cite{vykopal2017kypo}, CyTrONE~\cite{beuran2018cytrone}) that model flat networks, CityShield deploys three isolated Docker network segments: management, cyber range, and IoT range. This layout reflects the topology of real smart city deployments. Researchers can simulate attack chains that cross domain boundaries (e.g., network reconnaissance $\rightarrow$ IoT sensor compromise $\rightarrow$ traffic system manipulation), which cannot be reproduced on ranges confined to a single domain.

    \item \textbf{A detection engine combining 11 specialized evaluators with a generic threshold evaluator, covering 76 ATT\&CK techniques and extensible without code changes.} The specialized evaluators implement aggregation logic tailored to specific domains (port cardinality, byte sums, fan-in/fan-out), while the generic threshold evaluator covers 54 additional rules through YAML parameterization. Under controlled evaluation against 690,287 total events (689,172 background plus 1,115 injected) across 15 attack campaigns, all 75 rules triggered successfully. This validates both rule correctness and query-pipeline integrity, with a 4.2\% false positive rate. Compared to a baseline using only generic thresholds (Section~\ref{subsec:baseline}), the specialized evaluators reduce false positives by 62\%. This demonstrates that domain-aware aggregation logic yields measurable precision gains over simple counting.

    \item \textbf{An automated response pipeline that completes orchestration in under one minute, with safeguards enforced through per-rule policies.} The response manager dispatches 12 containment actions through Ansible with a measured MTTR of 38.4 seconds (worst case 60 seconds). Policies defined on each rule cover severity thresholds, rate limits, cooldown periods, and CIDR whitelisting to prevent response storms during sustained attacks. Every action is recorded in an append-only audit index, establishing three levels of traceability (log $\rightarrow$ alert $\rightarrow$ audit) for post-incident forensics. The current deployment executes simulated containment. However, the orchestration pipeline is architecturally identical to a production system and supports direct playbook replacement.

    \item \textbf{A quantitative evaluation methodology based on controlled adversarial emulation, internal baseline comparison, and explicit validity bounds.} We evaluate detection coverage through a structured campaign of 1,064 attack events and 51 false-positive seed events, with a comparison against a baseline using only generic thresholds. This methodology exposed a critical implementation bug: a \texttt{KEYWORD\_FIELDS} misconfiguration that had silently prevented 22 rules from triggering (see Section~\ref{sec:evaluation}). Scenario-based testing alone would not have revealed this defect. We explicitly characterize all reported metrics as upper-bound figures achievable under controlled injection and discuss the factors expected to degrade performance in real-world deployment.
\end{enumerate}
...
\textbf{Adversary capabilities.} We assume an external adversary who has gained network access to the smart city management network through compromised credentials, exposed services, or supply chain vectors. From this position the adversary may conduct reconnaissance, brute-force authentication, establish command-and-control channels, move laterally, exfiltrate data, and carry out impact operations such as ransomware, DDoS, or service disruption. These activities map to MITRE ATT\&CK tactics from Reconnaissance (TA0043) through Impact (TA0040), consistent with the NIST Cybersecurity Framework~\cite{nist_framework} Detect and Respond functions.
...
\textbf{Detection scope and limitations.} CityShield detects threats through log analysis at the network level rather than through endpoint agents. It identifies behaviors observable on the network, such as port scans, authentication failures, beaconing, and volume anomalies. It cannot detect techniques confined to the host (memory-resident malware, fileless attacks) that leave no network-visible traces. Physical attacks, social engineering beyond simulated phishing events, and zero-day exploits lacking network signatures are out of scope.
...
Cyber ranges provide controlled environments for security training and evaluation. Davis and Magrath~\cite{davis2013survey} surveyed the landscape and identified scalability and scenario realism as persistent challenges. KYPO~\cite{vykopal2017kypo} addresses provisioning through cloud-based OpenStack deployment with automated scenario management, but its scenarios target enterprise networks and do not model IoT or traffic infrastructure. CyTrONE~\cite{beuran2018cytrone} automates training content generation for virtualized networks. However, it lacks an integrated detection engine or response orchestration, so trainees can observe attacks but the platform does not evaluate detection effectiveness. Yamin et al.~\cite{yamin2020cyber} reviewed 30 cyber ranges and identified a gap in IoT and CPS-specific platforms, noting that most ranges model IT infrastructure exclusively.
...
\textbf{What existing cyber ranges lack.} Current ranges do not model smart city topologies spanning traffic, IoT, and network domains. They do not include built-in detection rule evaluation. And they do not provide automated response with policy enforcement and audit trails.

\subsection{Smart City Security Testbeds}

The SWaT~\cite{swat2016} and WADI~\cite{wadi2017} testbeds at iTrust provide physical ICS environments for water treatment and distribution. These generate real sensor data under attack, but they require specialized hardware, model a single ICS process, and cannot be reproduced outside their physical installation. MiniCPS~\cite{antonioli2017minicps} provides a lightweight CPS simulation toolkit but lacks detection and response components. Habibzadeh et al.~\cite{habibzadeh2019survey} surveyed smart city sensor security and identified the gap between theoretical threat models and practical testing infrastructure. Cui et al.~\cite{cui2018security} proposed a layered defense framework for smart city IoT but did not implement an operational testing platform.
...
SIEM platforms (Splunk, Elastic SIEM, IBM QRadar) provide log ingestion, rule-based detection, and dashboard visualization. Yet they do not include attack simulation or workflows intended for training. SOAR platforms~\cite{islam2019soar} automate playbook execution but require a pre-existing SIEM and do not ship with event generators or vulnerable training targets. MITRE Caldera~\cite{caldera2016} automates ATT\&CK adversary emulation through endpoint agents but provides neither network-level simulation nor IoT environments. The Sigma rule format~\cite{sigma2020} standardizes detection signatures but targets Windows event logs and enterprise SIEMs rather than smart city telemetry.

Mavroeidis and Bromander~\cite{mavroeidis2017cyber} proposed a cyber threat intelligence model aligned with ATT\&CK that informs our rule-to-technique mapping. Husak et al.~\cite{husak2019survey} surveyed network security situational awareness and identified the challenge of bridging detection to response, a gap that CityShield addresses through its integrated response manager.
...
Table~\ref{tab:comparison} compares CityShield with existing platforms across nine capabilities, including two (real exploitation and ICS protocol support) where CityShield falls short. Among the platforms surveyed, CityShield is the only one combining smart city simulation, integrated detection, automated response, and containerized deployment. It does not, however, perform real network exploitation or support industrial control system protocols.
...
CityShield deploys as 17 Docker Compose services organized across three isolated networks, with OpenSearch 2.11 serving as the centralized data store. Two design principles guided the architecture. First, training targets must be isolated from production services to prevent accidental disruption. Second, all telemetry must flow into a single searchable store so that detection rules can correlate events across domains.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Main Bridge Network}: Connects all core services, including the backend API, frontend, OpenSearch, three simulators, detection engine, response manager, scenario runner, Filebeat, and OpenSearch Dashboards.
...
Range loggers are dual-homed: they connect to both their isolated range network and the main bridge. This allows log forwarding without opening routing between ranges and production services.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Generation.} Three FastAPI simulators produce JSON events via background threads. Each event is written directly to its respective OpenSearch index (\texttt{logs-traffic}, \texttt{logs-iot}, \texttt{logs-network}) and also to a JSONL file on a shared Docker volume for Filebeat secondary ingestion.

    \item \textbf{Detection.} The detection engine polls at a configurable interval (default 30~s). For each of the 75 rules, it constructs an OpenSearch aggregation query scoped to a time window (default 300~s), executes it against \texttt{logs-*}, and compares bucket counts to thresholds. Matched alerts are written to the \texttt{alerts} index.

    \item \textbf{Response.} The response manager polls the \texttt{alerts} index every 30 seconds for open alerts at high or critical severity. It validates auto-response conditions defined on each rule, executes Ansible playbooks as subprocesses, and writes audit entries.
    ...
    \item \textbf{Audit.} Every response action, whether automated or manual, is recorded in \texttt{action-audit-log} together with the execution type, the triggering user, parameters, playbook stdout/stderr, and timestamps.
\end{enumerate}
...
We adopt a \emph{controlled adversarial emulation} approach analogous to MITRE Caldera's~\cite{caldera2016} technique-level simulation, but targeting network-level telemetry rather than endpoint agents. The methodology has four phases: (1)~establish a background traffic baseline using the three domain simulators; (2)~inject attack events crafted to match specific rule definitions; (3)~seed benign but ambiguous events to test false positive boundaries; and (4)~measure detection latency, response latency, rule coverage, and false positive rate. All metrics are derived from OpenSearch timestamps (\texttt{@timestamp} on log events, \texttt{triggered\_at} on alerts, \texttt{completed\_at} on audit entries).
...
\textbf{Background traffic.} Three simulators ran continuously for approximately 72 hours, generating 689,172 background events: 228,725 traffic events (33.2\%), 338,560 IoT events (49.1\%), and 121,887 network events (17.7\%). This distribution reflects the sensor-dense nature of smart city deployments where IoT telemetry dominates. Including attack and false-positive seed events, the total corpus comprises 690,287 events.

\textbf{Attack campaigns.} We executed 15 distinct attack campaigns comprising 1,064 attack events injected into the three \texttt{logs-*} indices across all 13 MITRE ATT\&CK Enterprise tactics. The campaigns cover brute force, port scanning, C2 beaconing, lateral movement, data exfiltration, credential attacks, defense evasion, execution/persistence, IoT sensor manipulation, impact attacks, web exploits, phishing, and privilege escalation. Events were injected in batches during the final 24 hours of the evaluation period, interleaved with ongoing background traffic.

\textbf{False positive seeding.} To evaluate discriminative precision, we injected 51 benign-but-ambiguous events: admin health checks resembling port scans (8 events), scheduled backup traffic resembling exfiltration (6 events), configuration management activity matching persistence patterns (10 events), legitimate authentication failures (9 events), periodic monitoring traffic resembling C2 beaconing (8 events), and sensor calibration events resembling IoT anomalies (10 events).

\subsection{Attack Emulation Engine}

The emulation engine generates structured attack events and writes them directly to OpenSearch. Four specialized classes cover the most common attack patterns: \texttt{BruteForceAttack} (T1110, with configurable attempt count and delay), \texttt{PortScanAttack} (T1046, SYN/CONNECT/UDP probes across configurable port ranges), \texttt{C2BeaconingAttack} (T1071, configurable interval with $\pm$20\% jitter to model realistic beacon variance), and \texttt{DataExfiltrationAttack} (T1041, staging followed by chunked 10~MB transfers). A fifth class, \texttt{GenericMitreAttack}, supports 27 additional techniques through tactic-specific event profiles. Every injected event carries \texttt{is\_attack: true} and a \texttt{correlation\_id} for post-hoc analysis.

\textbf{Schema validation.} We validated that the emulated event patterns correspond to real tool output by comparing the \texttt{PortScanAttack} log schema against nmap SYN scan output parsed through Zeek connection logs. Both produce per-connection records with source/destination IP-port tuples, protocol, and status fields. Similarly, the \texttt{BruteForceAttack} event structure mirrors SSH authentication failure entries in syslog format. Two caveats apply: (i)~structural schema correspondence does not guarantee detection of real exploitation traffic, which exhibits greater variance in timing, payload content, and evasion behavior; and (ii)~the emulated events lack the noise characteristics of real network captures (fragmented packets, retransmissions, concurrent legitimate traffic from the same source).

\subsection{Baseline Definition}

To contextualize the benefit of combining specialized and generic evaluators, we define an internal baseline: \emph{generic threshold detection only}. In this configuration, all 75 rules use the generic event-threshold evaluator, which groups events by \texttt{src\_ip}, counts events matching specified \texttt{event\_types}, and triggers when the count exceeds a threshold. This baseline removes the 11 specialized evaluators (cardinality, byte sums, fan-in/fan-out) and replaces them with simple count logic.
...
The backend (FastAPI, Python 3.11, port 8000) serves as the integration point between the frontend and all other services. It exposes 15 route modules under \texttt{/api/*}, covering authentication, alerts, rules, scenarios, logs, metrics, users, devices, health, WebSocket, research labs, actions, attack proposals, and MITRE technique queries. On startup, it seeds default users, 75 detection rules (from a bundled JSON file), vulnerable target asset records, and OWASP Top~10~\cite{owasp2021} scenario templates. This means the platform is immediately usable after a fresh deployment without manual data entry. Authentication relies on JWT with HS256 and bcrypt-hashed passwords stored in the \texttt{users} OpenSearch index.

\subsection{Data Store}

We chose OpenSearch 2.11 over a traditional RDBMS because its full-text search, aggregation framework, and flexible schema suit the heterogeneous, high-volume telemetry produced by the simulators. The data store comprises 13 indices: \texttt{users}, \texttt{rules}, \texttt{scenarios}, \texttt{scenario\_runs}, \texttt{alerts}, \texttt{city-assets}, \texttt{logs} (template), \texttt{logs-traffic}, \texttt{logs-iot}, \texttt{logs-network}, \texttt{blocked\_ips}, \texttt{attack-proposals}, and \texttt{action-audit-log}. All indices carry explicit field mappings (keyword, date, ip, integer, object types) defined programmatically at startup to ensure consistent query behavior.
...
For hands-on training, CityShield deploys real targets: Metasploitable2 on the isolated cyber range, an attacker host equipped with nmap and netcat, an IoT target exposing HTTP and MQTT-like endpoints on the isolated IoT range, and per-user Ubuntu lab containers provisioned through the backend Docker API and connected to all three networks.
...
\IfFileExists{cityshield_dashboard.jpeg}{%
    ...
    \framebox[\columnwidth]{\parbox{0.9\columnwidth}{\centering\vspace{1.5cm}\textit{Screenshot placeholder --- add \texttt{cityshield\_dashboard.png} to compile.}\vspace{2cm}}}%
}
...
\node[admbox]   (d4a) at (-3, -15.0) {User Mgmt};
\node[admbox]   (d4b) at (-1, -15.0) {Device Mgmt};
\node[admbox]   (d4c) at (1, -15.0) {Proposals};
\node[admbox]   (d4d) at (3, -15.0) {Rule Oversight};
\node[admbox]   (d5) at (0, -16.2) {Monitoring};
\node[admbox]   (d6a) at (-2.7, -17.3) {OpenSearch Dashboards};
\node[admbox]   (d6b) at (0, -17.3) {MTTD/MTTR Metrics};
\node[admbox]   (d6c) at (2.3, -17.3) {Audit Log};
...
Each of the 75 rules specifies a unique ID, a severity level (informational through critical), target log source indices, match logic (type and parameters), a MITRE ATT\&CK technique mapping, recommended response actions, and optional auto-response configuration. We chose YAML over a database representation because it supports version control, peer review, and offline authoring. These are workflows that detection engineers are already accustomed to.
...
\begin{itemize}[leftmargin=*,nosep]
    \item \texttt{net\_scan}: Cardinality aggregation on \texttt{dst\_port} per \texttt{src\_ip}. Triggers when the distinct port count exceeds the threshold. This detects horizontal port scanning without requiring individual connection logs.
    ...
    \item \texttt{lateral\_movement}: Terms on \texttt{src\_ip} with cardinality on \texttt{dst\_ip}. This is the inverse of DDoS detection: it identifies a single host contacting many destinations.
    \item Additional types: \texttt{iot\_anomaly}, \texttt{dos\_endpoint}, \texttt{ransomware}, \texttt{web\_exploit}, \texttt{credential\_dump}.
\end{itemize}

\textbf{Generic threshold evaluator} (covering 54 rules) groups events by a configurable field, counts the events in each bucket, and triggers when the count exceeds a threshold. It implements 15 match logic types, including \texttt{event\_threshold}, \texttt{log\_clearing}, \texttt{cmd\_execution}, \texttt{process\_injection}, and \texttt{defense\_evasion}. Adding a new ATT\&CK technique requires only a YAML file specifying the relevant event types, group-by field, and threshold. No code changes are needed. The trade-off is lower specificity: a generic rule may fire on unrelated event surges if thresholds are set too low.
...
Table~\ref{tab:mitre_coverage} summarizes coverage by tactic. The 75 rules cover 13 of the 14 ATT\&CK Enterprise tactics. Resource Development (TA0042) is not covered because it involves pre-compromise activities (acquiring infrastructure, developing capabilities) that do not produce network-observable events within the platform's detection scope.
...
To prevent false positives from authorized testing sources, the engine enforces whitelisting at two layers using CIDR notation. At the detection layer, source IPs are checked against the whitelist defined on each rule before any alert is written. At the response layer, the response manager independently verifies the whitelist a second time before executing any playbook.

\subsection{Response Action Catalog}

Twelve response actions are mapped to Ansible playbooks through a YAML configuration file (\texttt{playbooks\_map.yml}). In the current deployment, these playbooks perform simulated containment (e.g., logging ``SIMULATED: Blocking IP at firewall'') rather than modifying real infrastructure. The orchestration pipeline itself, including parameter extraction, subprocess invocation, stdout capture, and audit logging, is the same as it would be in production. This means the simulated playbooks can be swapped for real infrastructure commands without architectural changes.
...
Each detection rule may carry an auto-response configuration that enforces several conditions: a minimum severity threshold, optional threat intelligence enrichment (AbuseIPDB integration), a maximum number of executions per hour, a cooldown period between consecutive executions, a subset of allowed actions, and whitelisted subnets in CIDR notation. Together, these conditions prevent response storms. For instance, a DDoS alert that generates hundreds of duplicate detections is limited to a configurable number of \texttt{block\_ip} executions per hour.

\subsection{Response Execution Flow}

The response manager processes each alert in a fixed sequence:
...
We evaluate CityShield along four dimensions: (1)~quantitative detection and response metrics from the controlled adversarial emulation, (2)~per-scenario timing analysis, (3)~comparison against a baseline using only generic thresholds, and (4)~analysis of detection coverage gaps. All experiments ran on a single machine (Docker Desktop, Apple M-series, 16~GB RAM) with all 17 services active.

\subsection{Bug Discovery and Retest}

During the initial evaluation run, 22 of 75 rules failed to trigger. Root-cause analysis traced the problem to a \texttt{KEYWORD\_FIELDS} misconfiguration that prevented OpenSearch from aggregating on fields such as \texttt{user\_id}, \texttt{host\_id}, and \texttt{device\_id}. After fixing this bug and adding a missing handler for the \texttt{threshold} match-logic type, we reran the full attack injection. All metrics reported below reflect the post-fix run. This experience validates the evaluation methodology itself: a controlled emulation that exercises every rule individually can surface implementation defects that scenario-level testing alone would miss.

\subsection{Detection and Response Metrics}

Table~\ref{tab:detection-performance} presents the aggregate detection metrics.

\textbf{Why 100\% rule utilization is expected.} The injected events are constructed to match each rule's required event types, group-by fields, and count thresholds. Because of this, 100\% rule utilization (75/75) is an expected outcome of correct implementation rather than evidence of detection capability. What this result confirms is that the entire detection pipeline (YAML parsing, OpenSearch query construction, aggregation execution, threshold comparison, and alert generation) functions correctly for every rule. Had any rule failed to trigger, the cause would be an implementation defect rather than a detection limitation. An additional 12 rules embedded in the attack execution engine, covering techniques not addressed by the 75 YAML rules, also fired. This brings the total to 87 distinct rules, 76 unique MITRE techniques, and 14 tactics (the 13 Enterprise tactics from Table~\ref{tab:mitre_coverage} plus Reconnaissance events from the attack engine).
...
Table~\ref{tab:baseline} shows that both configurations achieve 100\% rule utilization, as expected, since the injected events exceed thresholds regardless of evaluator type. The important difference lies in the false positive rate. The generic baseline produces an 11.1\% FPR overall, with particularly high rates for reconnaissance (9.4\%) and exfiltration (7.2\%). The reason is straightforward: rules that merely count events cannot distinguish a port scan (many destination ports from one source) from a legitimate service contacting many ports. Only cardinality aggregation on \texttt{dst\_port} makes this distinction. Similarly, the byte-sum evaluator for exfiltration separates high-volume legitimate transfers (backups, synchronization) from attack staging by examining total byte count rather than event count alone.

The generic baseline does achieve a marginally lower MTTD (16.2~s vs.\ 16.8~s) because its simpler queries execute faster. But this 0.6-second advantage is negligible relative to the 30-second polling interval and comes at the cost of 2.6$\times$ more false positives. The 62\% reduction in false positives for reconnaissance and exfiltration categories is the primary quantitative contribution of the architecture. It shows that domain-aware aggregation logic provides a measurable precision gain that holds regardless of the controlled evaluation setting.

\textbf{Comparison with Sigma rules.} Our YAML rule format bears a structural resemblance to Sigma's generic signature format~\cite{sigma2020}. Both specify a log source, detection logic, and severity. The difference is in scope: Sigma rules target Windows Event Logs and enterprise SIEM backends (Splunk, Elastic), whereas CityShield rules operate on smart city event types (\texttt{sensor\_anomaly}, \texttt{traffic\_signal\_change}) that have no Sigma equivalent. The specialized evaluators (cardinality, byte sums) also express aggregation logic that Sigma's field-match-and-count model cannot natively represent.

\subsection{Detection Coverage Analysis}

Table~\ref{tab:mitre_coverage} shows that the 75 rules cover 13 of 14 ATT\&CK Enterprise tactics. The controlled evaluation confirms that every rule is syntactically correct, properly parsed, and capable of generating an alert when matching events are present. This is a necessary validation given that the pipeline involves YAML parsing, dynamic OpenSearch query construction, and aggregation whose behavior depends on field types. The 21 rules using specialized evaluators produce structured evidence (source IPs, port counts, byte volumes), while the 54 generic rules provide breadth of coverage at the cost of lower discriminative power.

\textbf{Coverage gaps.} No rules target ATT\&CK for ICS techniques (Modbus function code abuse, DNP3 unauthorized commands), which are relevant to smart city SCADA systems. Resource Development (TA0042) is not covered because pre-compromise activities occur outside the platform's observation boundary.

\subsection{Automated Response Evaluation}

Of the 75 detection rules, only 6 had auto-response enabled during evaluation. This deliberately narrow scope reflects the platform's training orientation: auto-response is off by default to prevent unintended containment during exercises, and a Researcher or Administrator must enable it on each rule individually. The 6 enabled rules cover brute force, DDoS, lateral movement, data exfiltration, ransomware, and network scanning. These are the high-severity, high-confidence cases most suitable for automation. The response manager executed 461 actions (\texttt{block\_ip}, \texttt{isolate\_service}, \texttt{quarantine\_host}, \texttt{network\_segmentation}) and resolved 313 alerts (14.7\%). The remaining 85.3\% stayed open by design. The measured alert-to-action latency is $21.6 \pm 3.2$~s, dominated by how each alert aligns with the response manager's polling cycle.
...
CityShield's detection engine is entirely rule-based: 75 threshold and aggregation rules evaluated on a fixed polling cycle. This is a deliberate trade-off. On a training platform, rule transparency is a feature rather than a limitation. Trainees can read, modify, and understand every detection decision, something that opaque ML models do not allow. That transparency, however, comes with fundamental detection limits.

\textbf{No novelty detection.} Rules that rely on thresholds detect known patterns. They cannot identify novel attack behaviors, polymorphic malware, or zero-day techniques that leave no known signature. The 54 generic rules share identical counting logic and differ only in their parameters, which limits their discriminative power on ambiguous traffic. The baseline comparison (Table~\ref{tab:baseline}) quantifies this gap: generic threshold rules produce 2.6$\times$ more false positives than the specialized evaluators because they lack the aggregation specificity to distinguish, for example, a port scan from a legitimate service that contacts multiple ports.

\textbf{Static thresholds vs.\ adaptive baselines.} All rule thresholds are fixed values (e.g., 5 failures for brute force, 10 distinct ports for scan detection). In real smart city deployments, traffic baselines shift with time of day, season, and infrastructure changes. Adaptive approaches such as autoencoders trained on IoT sensor profiles~\cite{habibzadeh2019survey} or LSTM networks for temporal sequence analysis would complement rule-based detection by flagging deviations from learned normal behavior.

\textbf{Pedagogical value.} The rule-based approach enables trainees to learn detection engineering by tuning thresholds, adding rules, and observing the FPR-sensitivity trade-off directly. A future hybrid architecture combining rules with ML would enable comparative evaluation exercises.

\subsection{Polling vs.\ Streaming: Architectural Trade-offs}

The 30-second polling architecture bounds MTTD to $\leq T_p$ (worst case) with a measured average of 16.8~s. For a training and evaluation platform, this latency is acceptable because trainees focus on detection logic, not sub-second response. For production smart city deployments, however, several considerations arise.

\textbf{Latency-sensitive domains.} Traffic signal manipulation or sensor data poisoning can cause physical harm within seconds. A 30-second worst-case MTTD may be too slow for safety-critical applications. A streaming architecture built on OpenSearch change-data-capture or Apache Kafka would bring MTTD below one second, though at the cost of greater infrastructure complexity.

\textbf{Scalability under load.} The current architecture evaluates all 75 rules sequentially within each polling cycle. At 75 queries $\times$ $<$10~ms per query, the total computation is $<$750~ms per cycle, well within the 30-second budget. Scaling to thousands of rules or sub-second polling would require parallelized rule evaluation.

\subsection{Scalability and Real Deployment Challenges}

\textbf{Single-node limitation.} The current deployment runs on a single OpenSearch node that processed 689,172 events. A real smart city with hundreds of thousands of IoT sensors, traffic cameras, and network devices would produce orders of magnitude more data. Such a deployment would require a multi-node OpenSearch cluster with index sharding and tiered storage.
...
\textbf{Integration with real infrastructure.} CityShield generates simulated telemetry. Connecting it to real infrastructure (SCADA, building management, traffic control) would require protocol adapters for Modbus, DNP3, OPC~UA, and MQTT, none of which the current platform implements.
...
\textbf{L1: Controlled emulation, not real exploitation.} The 100\% rule utilization reflects ideal conditions in which injected events conform to rule definitions by construction. This metric validates pipeline correctness (YAML parsing, query construction, threshold comparison) rather than real-world detection capability. Under genuine exploitation traffic, we expect detection rates to degrade, especially for rules that rely on exact event-type matches rather than behavioral aggregation.

\textbf{L2: Controlled FP injection.} The 4.2\% FPR derives from 51 deliberately crafted ambiguous events chosen to test known rule boundary conditions. This controlled set does not capture the full diversity of benign traffic that might trigger rules in operational environments (e.g., load balancer health checks, CDN probes, automated vulnerability scanners). Under real mixed traffic the FPR would likely be higher. The baseline comparison provides a partial bound: a detector using only generic thresholds achieves 11.1\% FPR on the same event corpus, suggesting an operational range of 4--12\%.
...
\textbf{L5: Severity distribution skew.} The alert distribution (critical 39.7\%, high 53.5\%, medium 6.8\%) is amplified by high-frequency reconnaissance rules. Production deployments would require low-severity informational rules and per-rule alert deduplication.
...
\textbf{L7: Self-evaluation.} All experiments were designed and executed by the platform developers. This is a common limitation in systems research but one that reduces external validity. Independent validation by external security teams or through adversarial red-team engagement would provide stronger evidence.

\subsection{Broader Constraints}

\textbf{Enterprise ATT\&CK only.} All 75 rules map to the ATT\&CK Enterprise matrix~\cite{mitre_attack}. The ATT\&CK for ICS matrix covers over 80 techniques specific to industrial protocols that are directly relevant to smart city SCADA systems. This gap is significant because smart cities increasingly integrate operational technology alongside IT infrastructure.

\textbf{Simulated, not operational, environment.} Although CityShield deploys real containerized services, its simulators generate synthetic telemetry that lacks the noise, scale, and variability of production infrastructure. The platform therefore validates detection \emph{logic} but not detection \emph{effectiveness} against the full complexity of operational traffic.
...
\textbf{Audit integrity.} The \texttt{action-audit-log} index is append-only by application convention. OpenSearch does not enforce write-once semantics, so a privileged user could modify audit records. Production deployments should use index lifecycle policies with read-only transitions.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Hybrid detection}: Integrate unsupervised anomaly detection (autoencoders for IoT baselines, LSTM for temporal sequences) alongside rule-based detection. This would enable trainees to compare detection paradigms on identical attack corpora.
...
CityShield demonstrates that a fully containerized platform can model the attack surface of smart city infrastructure across multiple domains while providing end-to-end detection and automated response with measurable performance. Under controlled adversarial emulation against 690,287 total events and 15 attack campaigns, the platform validates pipeline correctness (75/75 rules triggered), covers 76 MITRE ATT\&CK techniques across 14 tactics, and maintains a 4.2\% false positive rate with 16.8-second mean detection latency and 38.4-second mean response latency. The most important quantitative finding is that combining specialized evaluators with generic thresholds reduces false positives by 62\% compared to using thresholds alone. This result holds independently of the controlled evaluation setting and underscores the value of domain-aware aggregation logic.

The platform brings together three functions that are ordinarily spread across separate tools: smart city simulation (replacing physical testbeds), detection aligned with ATT\&CK (replacing standalone SIEM configurations), and automated response under per-rule policies (replacing manual playbook execution). All of this runs from a single \texttt{docker compose up} deployment. New techniques can be added by writing a YAML rule file without modifying any code, and the Ansible playbook interface allows direct substitution of production infrastructure commands.

We have been candid about the evaluation's limitations. The reported metrics represent upper-bound performance under controlled conditions. The platform lacks ICS protocol coverage. No independent user study validates its training effectiveness. These limitations point to concrete future work: real adversary integration through Caldera, hybrid ML detection, ATT\&CK for ICS coverage, and controlled user studies with $n \geq 30$ participants. None of these require architectural changes. CityShield provides a reproducible, open-source foundation for this continued research.
